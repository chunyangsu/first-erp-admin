<template>
  <div>
    <el-table :data="roleData" stripe border style="width: 100%">
      ...
      <el-table-column prop="roleDesc" label="描述" min-width="180">
      </el-table-column>
      <el-table-column prop="address" label="操作" min-width="200" fixed="right">
        <template slot-scope="scope">
          <!-- 修改按钮 -->
          <el-button type="primary" size="mini" plain icon="el-icon-edit" @click.native="$refs.RolesDialog.showEditRole(scope.row)"></el-button>
          <!-- 删除按钮 -->
          <el-button type="danger" size="mini" plain icon="el-icon-delete" @click="delRole(scope.row.id)"></el-button>
          <!-- 分配权限 -->
    ...
    <!-- 修改角色对话框 -->
    <roles-dialog ref="RolesDialog" :children="children"</roles-dialog>
  </div>
</template>
