<template>
  <div v-clickoutside="closeDialog">
    <div v-if="remarkVisible" class="container">
      <div id="remarksScroll" style="max-height: 350px; overflow-y: auto;">
        <div v-for="item in data" :key="item.id" class="remark">
          <!-- 单条备注 -->
          <div class="info">
            <!-- 左侧头像 -->
      ...
      <!-- 无数据时 暂无备注提醒 -->
      <div v-if="data.length === 0" style="text-align: center; padding: 10px 0 5px 0; font-size: 15px;">暂无备注</div>
      <!-- 输入框部分 -->
    ...
    <!-- 默认展示的最新一条备注 -->
    <div style="cursor: pointer;" @click="showDialog">{{ remark }}</div>
  </div>
</template>
