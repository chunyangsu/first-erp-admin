<template>
  <div>
    <el-drawer :visible.sync="drawer" :direction="direction" :modal="false" size="40%">
      <div class="content">
        <!-- 头部切换方向组件 -->
        <div class="header">
          <el-radio-group v-model="direction">
            <el-radio label="ltr">从左侧弹出</el-radio>
            <el-radio label="rtl">从右侧弹出</el-radio>
          </el-radio-group>
        </div>
        <!-- 展示部分 -->
        <div v-loading="LD_remark" ref="remarksScroll" class="middle">
          <div v-for="item in remarkList" :key="item.id">
            <!-- 单条备注 -->
            <div class="one_remark">
              <!-- 左侧头像 -->
              <div class="left_img">
                <img src="../../assets/images/kaxikai1.jpg" class="avatar" width="38" height="38">
              </div>
              <!-- 右侧文本 -->
              <div class="right_text">
                <!-- 姓名和时间 -->
                <div class="name_time clearfix">
                  <div class="name">
                    <span @click="reply(item)">{{ item.user_name }}</span>
                  </div>
                  <div class="date">{{ item.time }}</div>
                </div>
                <!-- 备注内容 -->
                <pre class="text_content"><span v-html="item.content"/></pre>
              </div>
            </div>
          </div>
          <!-- 无数据时 暂无备注提醒 -->
          <div v-if="remarkList.length === 0" style="padding-top: 20px; text-align: center; font-size: 18px; color: #7f8082;">暂无备注</div>
        </div>
        <!-- 底部输入框部分 -->
        <div v-if="showInput" class="footer">
          <!-- input输入框 -->
          <div class="input_part">
            <el-form ref="dataForm" :rules="rules" :model="temp">
              <el-form-item prop="inputContent">
                <!-- 0：有输入值，需要传的  1：回复xxx  2：占位 -->
                <el-input v-if="placeholder_flag === 0" ref="remarkInput" v-model="temp.inputContent" :rows="2" placeholder="请输入内容" type="textarea" autofocus @input="searchUser" />
                <el-input v-if="placeholder_flag === 1" ref="remarkInput" v-model="temp.inputContent" :rows="2" :placeholder="replyHolder" type="textarea" autofocus @input="searchUser" />
                <!-- 占位，用于解决@xxx后点击确定按钮后，input框会展示html标签的闪现问题 -->
                <el-input v-if="placeholder_flag === 2" ref="remarkInput" v-model="placeholder_for_html" :rows="5" placeholder="请输入内容" type="textarea" autofocus @input="searchUser" />
              </el-form-item>
            </el-form>
          </div>
          <!-- 提交按钮 -->
          <div class="submit_button">
            <el-button type="primary" style="width: 60px; height: 54px; float: right;" @click="send">提交</el-button>
          </div>
          <!-- 用户下拉框 -->
          <div v-clickoutside="colseOption" v-if="optionVisible" class="userOption">
            <el-table :data="userList" :show-header="false" @cell-click="selected">
              <el-table-column>
                <template slot-scope="scope">
                  <span class="itemOption">{{ scope.row.name }}</span>
                </template>
              </el-table-column>
            </el-table>
          </div>
        </div>
      </div>
    </el-drawer>
  </div>
</template>

<script>
import Clickoutside from 'element-ui/src/utils/clickoutside'

export default {
  name: 'SgoRemark', // 备注组件
  directives: {
    Clickoutside
  },
  props: {
    // 是否展示input输入框
    showInput: {
      type: Boolean,
      default: true
    }
  },
  data() {
    return {
      drawer: false,
      direction: 'rtl',
      LD_remark: true,
      remarkList: [], // 备注数组
      temp: {
        inputContent: '' // 输入框内容
      },
      userInfo: {}, // 当前用户
      replyObjectId: undefined, // 回复对象的id
      userList: [], // 用户列表
      allUserList: [], // 完整的用户列表，用于删除搜索关键字后再次赋值给用户列表
      reminds: [], // @对象的id
      objectNameList: [], // @对象名称的数组
      optionVisible: false,
      placeholder_flag: 0, // 默认展示传值的那个input
      replyHolder: '',
      placeholder_for_html: '',
      rules: {
        inputContent: [
          {
            required: true,
            pattern: /^[\s\S]*.*[^\s][\s\S]*$/, // 非空校验：不可全为空格和换行，允许字符前面和后面为空
            message: '输入内容不能为空',
            trigger: 'change'
          }
        ]
      }
    }
  },
  methods: {
    // 滚动条至最底部
    scrollToBottom() {
      this.$nextTick(() => {
        var container = this.$refs.remarksScroll
        container.scrollTop = container.scrollHeight
      })
    },
    // 获取备注数据
    async getRemarks() {
      this.LD_remark = true
      this.remarkList = []
      const response = await this.$http.get('/comment/getCommentsList')
      const { data, meta } = response.data
      if (meta.status === 200) {
        this.remarkList = data.comments
        setTimeout(() => {
          this.scrollToBottom()
          this.LD_remark = false
        }, 500)
      }
    },
    // 弹出备注弹窗
    showRemarkDrawer() {
      this.drawer = true
      this.temp.inputContent = '' // 清空输入框
      this.getRemarks()
    },
    // 获取用户列表数据
    async getUserList() {
      const response = await this.$http.get('/users/getUsersList')
      const { data, meta } = response.data
      if (meta.status === 200) {
        this.userList = data.users
        this.allUserList = data.users // 将完整的用户列表存在另一个数组中
      }
    },
    // 点击空白处关闭用户下拉框
    colseOption() {
      this.optionVisible = false
    },
    // 用户下拉框选择事件
    selected(row, column, cell, event) {
      this.objectNameList.push(row.name) // 获取@对象的name，组成数组
      this.objectNameList = Array.from(new Set(this.objectNameList)) // 数组去重
      // 去掉@后面手动输入的字符串，以确保@后面紧跟着点击的下拉选项
      this.temp.inputContent = this.temp.inputContent.slice(0, this.temp.inputContent.lastIndexOf('@') + 1)
      this.temp.inputContent = this.temp.inputContent + row.name // 将@对象的名字展示在输入框内
      this.$nextTick(() => {
        this.$refs['remarkInput'].focus()
      })
      this.optionVisible = false
    },
    // 点击人名展示回复对象
    reply(row) {
      this.placeholder_flag = 1 // 回复xxx占位文本
      this.temp.inputContent = ''
      this.replyHolder = '回复' + row.user_name + '：'
      if (this.replyObjectId === undefined || this.replyObjectId !== row.id) {
        this.replyObjectId = row.id
      } else if (this.replyObjectId === row.id) {
        // 取消回复
        this.replyObjectId = undefined
        this.placeholder_flag = 0
      }
      this.$nextTick(() => {
        this.$refs['remarkInput'].focus()
      })
    },
    // 弹出下拉列表后输入关键字进行搜索
    searchUser(val) {
      // 判断最后一个字符是否是@，是的话弹出下拉列表
      if (val.charAt(val.length - 1) === '@') {
        this.optionVisible = true
        this.placeholder_flag = 0
        // 已经获取了用户列表数据，就不再调用接口了
        if (this.allUserList.length > 0) {
          this.userList = this.allUserList
        } else {
          this.getUserList()
        }
      }
      if (val.includes('@')) {
        // 获取@最后一次出现的位置(下标)并截取最后一个@及后面的字符串，+1 是从@后面一个字符开始算起(不包括@)
        const str = val.slice(val.lastIndexOf('@') + 1)
        if (str !== '') {
          this.userList = this.userList.filter(item => item.name.includes(str))
        } else {
          // 获取所有的用户列表
          this.userList = this.allUserList
        }
      } else {
        // input框内没有@时，关闭下拉框
        this.optionVisible = false
        this.placeholder_flag = 0
      }
    },
    replaceRemark(val, value) {
      const replaceReg = new RegExp(val, 'ig')
      const replaceString = '<span style="color: #2289f0;">' + val + '</span>'
      return value.replace(replaceReg, replaceString)
    },
    // 提交
    send() {
      this.placeholder_flag = 2 // 目的是点击确定按钮后，立即清空input框
      this.objectNameList.forEach(item => {
        if (this.temp.inputContent.includes(item)) {
          this.temp.inputContent = this.replaceRemark('@' + item, this.temp.inputContent)
        }
      })
      // 提交成功，给备注数据数组新增最新备注
      // const curTime = parseTime(Date.parse(new Date())) // 获取当前时间
      // 将新添加的备注push进数组里
      this.remarkList.push({
        // comment_user_icon: this.userIcon,
        user_name: '苏春阳',
        // comment_object_id: this.replyObjectId, // 回复对象的id
        // comment_object_name: this.replyObjectName, // 回复对象的名字
        // time: curTime,
        content: this.temp.inputContent
      })
      this.temp.inputContent = ''
      this.placeholder_flag = 0
      this.$nextTick(() => {
        this.scrollToBottom()
        this.$refs['dataForm'].clearValidate()
      })
    }
  }
}
</script>

<style scoped lang="less">
.content {
  width: 100%;
  height: 100%;
  display: flex;
  flex-direction: column;
  // header部分
  .header {
    width: 100%;
    height: 30px;
    background-color: white;
    padding-left: 12px;
  }
  // 中间内容部分
  .middle {
    width: 100%;
    height: calc(100vh - 172px);
    overflow: auto;
    // background-color: #f5f5f5;
    background-color: white;
    // border-top: 1px solid #e6e1e1;
    display: flex;
    flex-direction: column;
    padding: 12px;
    // 一条备注
    .one_remark {
      position: relative;
      display: flex;
      align-items: center;
      padding-top: 10px;
      padding-bottom: 10px;
      border-top: 1px solid #e6e1e1;
      // 左侧头像-图片
      .left_img {
        position: absolute;
        left: 0;
        top: 10px;
        border-radius: 50%;
        img {
          width: 38px;
          height: 38px;
          border-radius: 50%;
        }
      }
      // 左侧头像-姓名
      .lastTwoName {
        position: absolute;
        left: 0;
        top: 10px;
        border-radius: 50%;
        width: 38px;
        height: 38px;
        background: #409eff;
        color: #fff;
        line-height: 38px;
        font-size: 14px;
        text-align: center;
        overflow: hidden;
      }
      // 右侧文字
      .right_text {
        width: calc(100% - 50px);
        display: flex;
        flex-direction: column;
        margin-left: 50px;
        .name_time {
          padding-bottom: 5px;
          // 姓名
          .name {
            float: left;
            padding-right: 5px;
            font-size: 16px;
            color: rgb(34, 137, 240);
            cursor: pointer;
          }
          .reply {
            float: left;
            font-size: 15px;
            padding-right: 5px;
          }
          // 回复对象的名字
          .replyName {
            float: left;
            font-size: 15px;
            color: rgb(34, 137, 240);
          }
          // 时间
          .date {
            float: right;
            font-size: 14px;
            color: #909399;
          }
        }
        // 内容
        .text_content {
          // padding: 7px 0;
          margin: 0;
          // background-color: white;
          span {
            // padding: 8px;
            // padding-left: 10px;
            // background-color: white;
            // border: 1px solid #f1eeee;
            font-size: 15px;
            color: #303133;
            // 自动换行
            word-wrap: break-word;
            word-break: normal;
          }
        }
      }
    }
  }
  // footer部分
  .footer {
    width: 100%;
    height: 80px;
    background-color: #eceaea;
    padding: 6px;
    // border-top: 1px solid #e6e1e1;
    // position: relative;
    display: flex;
    flex-direction: row;
    // 输入框
    .input_part {
      width: calc(100% - 80px);
    }
    // 提交按钮
    .submit_button {
      padding-left: 20px;
      // width: 100%;
      // height: 40px;
      // position: absolute;
      // left: 0;
      // bottom: 15px;
      // padding-right: 25px;
    }
    // 用户下拉框
    .userOption {
      position: absolute;
      left: 10px;
      bottom: 73px;
      width: 150px;
      min-height: 37px;
      max-height: 200px;
      overflow-y: auto;
      background-color: white;
      border: 1px solid #e4e7ed;
      border-radius: 4px;
      box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);
      .itemOption {
        padding-left: 10px;
      }
    }
  }
}
</style>
<style>
/* 抽屉弹窗的右上角关闭按钮的样式 */
.el-drawer__header {
  margin-bottom: 0;
  padding: 5px 3px 4px;
}
/* input框样式 */
.footer .input_part .el-form-item {
  margin-bottom: 0;
}
</style>
